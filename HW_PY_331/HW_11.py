...
def get_cities_set_from_json(file_name):
    """
    Функция читает список городов России из файла cities_set.json
    :param file_name: файл со свиском городов России
    :return: возращает сет городов России
    """
...
def get_list_result_from_json(file_name_result):
    """
    Функция читает список результатов игры из файла result.json для выдачи информации счета перед началом игры
...
def check_city_user(city_random, user_input):
    """
    Функция проверки верного ввода пользователем города России на правильную букву
    :param city_random: город в России выбранный рандомно в начале игры, потом он заменяется на город выданный компьютером
...
def write_list_result_in_json(file_result, result_list_end):
    """
...
def main():
    ...
    # вывод счета в игре
    print(f'Вы выиграли {score_user} попыток!')
    ...
    while cities_set_2:
        ...
        # проверка названия города, введенного пользователем вхождение в сет городов
